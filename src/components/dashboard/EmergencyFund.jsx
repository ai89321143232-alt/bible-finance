...
export default function EmergencyFund({ totalBalance, transactions, formatCurrency }) {
  ...
  let statusIcon = 'bg-rose-500/15 text-rose-600';
  let message = 'Пора копить';

  if (months >= 6) {
    statusColor = 'text-emerald-600';
    statusIcon = 'bg-emerald-500/15 text-emerald-600';
    message = 'Отличная подушка';
  } else if (months >= 3) {
    statusColor = 'text-amber-600';
    statusIcon = 'bg-amber-500/15 text-amber-600';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-black/45 text-xs">Подушка безопасности</span>
      </div>
      ...
      {avgMonthly > 0 && (
        <div className="mt-3">
          <p className="text-black/45 text-xs">{message}</p>
          <p className="text-black/35 text-xs mt-1">
            Трат ~{formatCurrency(avgMonthly)}/мес · Баланс {formatCurrency(totalBalance)}
          </p>
        </div>
      )}
      {avgMonthly === 0 && (
        <p className="text-black/35 text-xs mt-3">Добавьте расходы для расчёта</p>
      )}
    </motion.div>
  );
}